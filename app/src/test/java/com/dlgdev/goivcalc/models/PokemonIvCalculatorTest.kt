...
class PokemonIvCalculatorTest {
    val calc = PokemonIvCalculator()

    val prov = PokemonProvider()
    val gyarados = prov.get(130)
    val alakazam = prov.get(65)
    val exeggutor = prov.get(103)
    val vaporeon = prov.get(134)
    val exeggcute = prov.get(102)
    ...
    @Test fun forFunsies() {
        ...
        println(calc.calcCp(alakazam, 30, 14, 14, 12))
        println(calc.calcHp(alakazam, 30, 14))
    }
}